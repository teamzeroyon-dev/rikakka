...
export function ProblemClient({ id }: { id: string }) {
 const problem=getProblem(id)??getProblem('lever-01')!; const [solved,setSolved]=useState(false); const [weights,setWeights]=useState('weights' in problem?problem.weights:[]); const [grams,setGrams]=useState(5)
 const solve=()=>{setSolved(true);markSolved(problem.id)}
 if(isLaunchProblem(problem)) return <main className="min-h-screen bg-background px-5 py-6 text-foreground"><div className="mx-auto flex w-full max-w-xl flex-col gap-5"><header className="flex items-center gap-3"><Link href="/" className="flex size-12 items-center justify-center rounded-lg border border-border" aria-label="もどる"><ArrowLeft /></Link><h1 className="text-xl font-bold">{problem.emoji} {problem.title}</h1></header><p className="rounded-2xl bg-card p-4 text-center leading-6 shadow-sm">{problem.prompt}</p>{solved?<SolvedOverlay problem={problem as never} next={getNextProblem(problem.id) as never} onRetry={()=>setSolved(false)}/>:<LaunchCourse lanes={problem.lanes} length={problem.courseLengthCm} maxStretch={problem.maxStretchCm} onSolved={solve}/>}</div></main>
