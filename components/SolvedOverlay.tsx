...
export function SolvedOverlay({ problem, next, onRetry }: { problem: Problem; next: Problem; onRetry: () => void }) {
 return <section className="flex flex-col gap-5 rounded-3xl border border-primary/20 bg-card p-6 shadow-lg" aria-live="polite">
  ...
  <div className="flex flex-col gap-2"><p className="text-sm font-bold text-muted-foreground">きみが 見つけたこと</p><p className="leading-6">{problem.solved.discovery}</p><p className="rounded-xl bg-accent p-3 font-mono text-sm">{problem.solved.formula}</p>{problem.solved.note && <p className="text-sm leading-6 text-muted-foreground">{problem.solved.note}</p>}</div>
