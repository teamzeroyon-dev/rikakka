...
export type BalanceProblem = { id: string; archetype: 'balance'; title: string; difficultyLabel: 'かんたん' | 'ふつう' | 'やってみる'; prompt: string; notches: number; weights: Weight[]; gramsRange?: { min: number; max: number; step: number }; solved: { discovery: string; formula: string; realWorldTitle: string; realWorldBody: string; realWorldVisual: RealWorldVisual; note?: string } }
...
export function isLaunchProblem(problem: Problem): problem is LaunchProblem { return problem.archetype === 'launch' }
